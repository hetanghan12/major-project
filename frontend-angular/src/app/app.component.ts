/**
 * CloudAI Smart Storage - Main App Component
 * =============================================
 * Premium light theme with sidebar navigation and AI assistant.
 * 
 * @author CloudAI Team
 */
      ...
      <aside class="sidebar" 
        ...
        <nav class="sidebar-nav">
          ...
          <a (click)="navigateToStarred(); mobileOpen.set(false)" 
             class="sidebar-nav-item"
             [class.active]="currentRoute === '/starred'">
          ...
          <a (click)="navigateToTrash(); mobileOpen.set(false)" 
             class="sidebar-nav-item"
             [class.active]="currentRoute === '/trash'">
        ...
        <div class="sidebar-footer">
          ...
          <div class="pro-card">
              ...
              <button class="upgrade-btn-pro">Upgrade</button>
          </div>
          ...
          <div class="storage-indicator-minimal">
            ...
            <div class="storage-bar h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
      ...
      <main class="main-content" [class.sidebar-collapsed]="sidebarCollapsed()">
        ...
        <header class="main-header">
          ...
          <div class="header-right">
            ...
            <!-- User Menu Dropdown -->
            <div *ngIf="showUserMenu()" class="absolute right-6 top-16 mt-2 p-2 rounded-xl card shadow-lg z-50">
              <button 
                (click)="logout()" 
...
export class AppComponent implements OnInit {
  ...
  // Storage - now using REAL data from backend via StorageService
  ...
  constructor(
    ...
    effect(() => {
      ...
      if (docs && docs.length > 0) {
        ...
        const percent = Math.round((totalBytes / totalGB) * 100);

        this.storagePercent.set(Math.min(percent, 100));
        this.usedStorage.set(this.formatFileSize(totalBytes));

        console.log(`📊 [Sidebar] Storage updated: ${this.formatFileSize(totalBytes)} (${docs.length} files)`);
      }
    });
   ...
   * This calculates storage from actual S3/database data, not frontend state
   */
  async loadStorageStats(): Promise<void> {
    try {
      console.log('📊 Loading storage stats...');

      // Always load documents first (needed for fallback)
      await this.documentService.loadDocuments();
      const docs = this.documentService.documents();

      // Try to get storage stats from backend API
      let stats = null;
      try {
        stats = await this.storageService.loadStorageStats();
      } catch (apiError) {
        console.warn('📊 Storage API call failed, using fallback:', apiError);
      }

      if (stats) {
        // Use backend stats (calculated from database)
        this.storagePercent.set(stats.percentUsed);
        this.usedStorage.set(stats.storageUsedFormatted);
        console.log(`📊 Storage (from API): ${stats.storageUsedFormatted} / ${stats.storageLimitFormatted} (${stats.percentUsed}%)`);
        console.log(`📊 File count: ${stats.fileCount}`);
      } else {
        // Fallback: Calculate from frontend document list
        const totalBytes = docs.reduce((sum, doc) => sum + (doc.fileSize || 0), 0);
        const totalGB = 5 * 1024 * 1024 * 1024; // 5 GB
        const percent = Math.round((totalBytes / totalGB) * 100);
        this.storagePercent.set(Math.min(percent, 100));
        this.usedStorage.set(this.formatFileSize(totalBytes));
        console.log(`📊 Storage (from frontend): ${this.formatFileSize(totalBytes)} / 5 GB (${percent}%)`);
        console.log(`📊 Calculated from ${docs.length} documents`);
      }
    } catch (error) {
      console.error('Failed to load storage stats:', error);
      // Set default values on complete failure
      this.storagePercent.set(0);
      this.usedStorage.set('0 B');
    }
  }
