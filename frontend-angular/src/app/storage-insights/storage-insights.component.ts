...
@Component({
  ...
  template: `
    <div class="animate-in bg-[var(--bg-main)] min-h-[calc(100vh-64px)] pb-12">
      <!-- Header -->
      <div class="px-8 mt-6 mb-8">
        <h1 class="text-3xl font-bold tracking-tight text-[var(--text-primary)] mb-2">Storage Insights</h1>
        <p class="text-[var(--text-muted)] text-[15px]">Analyze how your storage is used and find ways to optimize it.</p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-6 px-8">
        ...
        <div class="bg-[var(--bg-card)] rounded-[20px] p-7 shadow-sm border border-[var(--border-color)]">
          <div class="flex items-center gap-3 mb-8">
            <svg class="w-6 h-6 text-yellow-400 drop-shadow-sm" fill="currentColor" viewBox="0 0 20 20">
              <path d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"></path>
            </svg>
            <h3 class="font-bold text-[var(--text-primary)] text-[17px] m-0">File Type Breakdown</h3>
          </div>
          ...
          <div class="flex flex-col gap-6">
            ...
            <div>
              <div class="flex justify-between items-end mb-2 cursor-pointer group" (click)="toggleCategory('videos')">
                <div class="flex items-center gap-2">
                  <span class="text-[14px] text-[var(--text-secondary)] font-medium group-hover:text-purple-600 transition-colors">Videos</span>
                  <svg class="w-4 h-4 text-[var(--text-muted)] transition-transform duration-200" [class.rotate-180]="expandedCategory() === 'videos'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                  </svg>
                </div>
                <span class="text-[14px] text-[var(--text-muted)] font-medium">{{ formatFileSize(videosBreakdown().total) }}</span>
              </div>
              <div class="h-[6px] w-full bg-[var(--bg-elevated)] rounded-full overflow-hidden cursor-pointer" (click)="toggleCategory('videos')">
              ...
              <div class="overflow-hidden transition-all duration-300" [style.maxHeight]="expandedCategory() === 'videos' ? '300px' : '0'">
                 <div class="pt-3 pl-2 pb-1">
                   <div *ngIf="videosBreakdown().details.length === 0" class="text-[13px] text-[var(--text-muted)] italic">No video files found.</div>
                   <div *ngFor="let item of videosBreakdown().details" class="flex justify-between text-[13px] py-1.5 border-b border-[var(--border-color)] last:border-0 bubble-panel mb-1 px-3 rounded-lg">
                     <span class="text-[var(--text-secondary)] font-medium flex items-center gap-2.5">
                       <span class="w-1.5 h-1.5 rounded-full bg-purple-400"></span> {{ item.format }}
                     </span>
                     <span class="text-[var(--text-muted)]">{{ formatFileSize(item.size) }}</span>
                   </div>
                 </div>
              </div>
            </div>
            ...
            <div>
              <div class="flex justify-between items-end mb-2 cursor-pointer group" (click)="toggleCategory('images')">
                <div class="flex items-center gap-2">
                  <span class="text-[14px] text-[var(--text-secondary)] font-medium group-hover:text-indigo-600 transition-colors">Images</span>
                  <svg class="w-4 h-4 text-[var(--text-muted)] transition-transform duration-200" [class.rotate-180]="expandedCategory() === 'images'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                  </svg>
                </div>
                <span class="text-[14px] text-[var(--text-muted)] font-medium">{{ formatFileSize(imagesBreakdown().total) }}</span>
              </div>
              <div class="h-[6px] w-full bg-[var(--bg-elevated)] rounded-full overflow-hidden cursor-pointer" (click)="toggleCategory('images')">
              ...
              <div class="overflow-hidden transition-all duration-300" [style.maxHeight]="expandedCategory() === 'images' ? '300px' : '0'">
                 <div class="pt-3 pl-2 pb-1">
                   <div *ngIf="imagesBreakdown().details.length === 0" class="text-[13px] text-[var(--text-muted)] italic">No image files found.</div>
                   <div *ngFor="let item of imagesBreakdown().details" class="flex justify-between text-[13px] py-1.5 border-b border-[var(--border-color)] last:border-0 bubble-panel mb-1 px-3 rounded-lg">
                     <span class="text-[var(--text-secondary)] font-medium flex items-center gap-2.5">
            ...
            <div>
              <div class="flex justify-between items-end mb-2 cursor-pointer group" (click)="toggleCategory('documents')">
                <div class="flex items-center gap-2">
                  <span class="text-[14px] text-[var(--text-secondary)] font-medium group-hover:text-green-600 transition-colors">Documents</span>
                  <svg class="w-4 h-4 text-[var(--text-muted)] transition-transform duration-200" [class.rotate-180]="expandedCategory() === 'documents'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                  </svg>
                </div>
                <span class="text-[14px] text-[var(--text-muted)] font-medium">{{ formatFileSize(documentsBreakdown().total) }}</span>
              </div>
              <div class="h-[6px] w-full bg-[var(--bg-elevated)] rounded-full overflow-hidden cursor-pointer" (click)="toggleCategory('documents')">
              ...
              <div class="overflow-hidden transition-all duration-300" [style.maxHeight]="expandedCategory() === 'documents' ? '400px' : '0'">
                 <div class="pt-3 pl-2 pb-1">
                   <div *ngIf="documentsBreakdown().details.length === 0" class="text-[13px] text-[var(--text-muted)] italic">No document files found.</div>
                   <div *ngFor="let item of documentsBreakdown().details" class="flex justify-between text-[13px] py-1.5 border-b border-[var(--border-color)] last:border-0 bubble-panel mb-1 px-3 rounded-lg">
                     <span class="text-[var(--text-secondary)] font-medium flex items-center gap-2.5">
        ...
        <div class="bg-[var(--bg-card)] rounded-[20px] p-7 shadow-sm border border-[var(--border-color)]">
          <div class="flex items-center gap-3 mb-6">
            ...
            <h3 class="font-bold text-[var(--text-primary)] text-[17px] m-0">Quick Actions</h3>
          </div>

          <!-- Deep Clean Recommended Card -->
          <div class="bg-[var(--bg-elevated)] border border-[var(--border-color)] rounded-[16px] p-5 relative overflow-hidden group">
            <h4 class="font-bold text-[var(--text-primary)] text-[15px] mb-1 relative z-10">Deep Clean Recommended</h4>
            <p class="text-[13.5px] text-[var(--text-secondary)] mb-5 relative z-10 leading-relaxed">
            ...
            <div class="absolute -right-6 -bottom-6 w-24 h-24 bg-purple-500/10 rounded-full blur-xl group-hover:bg-purple-500/20 transition-colors pointer-events-none"></div>
          </div>

          <!-- Large Files Card -->
          <div class="mt-6 border border-[var(--border-color)] rounded-[16px] p-5">
             <h4 class="font-bold text-[var(--text-primary)] text-[15px] mb-3">Large Documents</h4>
             <div class="flex flex-col gap-3">
               <div *ngIf="largeFiles().length === 0" class="text-[13px] text-[var(--text-muted)] italic">No large files found.</div>
               <div *ngFor="let file of largeFiles()" class="flex justify-between items-center text-[13px] bg-[var(--bg-main)] p-2.5 rounded-xl border border-[var(--border-color)]">
                  <div class="flex flex-col min-w-0 pr-3">
                    <span class="truncate font-medium text-[var(--text-secondary)]" [title]="file.fileName">{{ file.fileName }}</span>
                    <span class="text-[var(--text-muted)] text-xs mt-0.5" *ngIf="file.uploadedAt">{{ file.uploadedAt | date:'mediumDate' }}</span>
                  </div>
                  <span class="bg-[var(--bg-elevated)] text-[var(--text-primary)] font-medium px-2 py-1 rounded-md">{{ formatFileSize(file.fileSize || 0) }}</span>
               </div>
             </div>
          </div>
          ...
          <div class="mt-6 border border-[var(--border-color)] rounded-[16px] p-5">
             <div class="flex justify-between items-start mb-2">
               <div>
                 <h4 class="font-bold text-[var(--text-primary)] text-[15px]">Duplicate Files</h4>
                 <p class="text-[12.5px] text-[var(--text-muted)] mt-0.5" *ngIf="duplicateFiles().length > 0">Select duplicates to safely free up space.</p>
               </div>
               <span class="text-xs bg-red-500/10 border border-red-500/20 text-red-500 px-2 py-0.5 rounded-full font-bold" *ngIf="duplicateFiles().length > 0">{{ duplicateFiles().length }} items</span>
             </div>
             
             <div class="text-[13px] text-[var(--text-muted)] italic mb-1 mt-4" *ngIf="duplicateFiles().length === 0">No duplicates found. Great job!</div>
             
             <div class="flex flex-col gap-2 max-h-[160px] overflow-y-auto mb-4 mt-4 pr-1" *ngIf="duplicateFiles().length > 0">
               <label *ngFor="let file of duplicateFiles()" class="flex items-start gap-3 p-2.5 bg-[var(--bg-main)] hover:bg-[var(--bg-hover)] rounded-xl cursor-pointer border border-[var(--border-color)] transition-all shadow-sm">
                 <input type="checkbox" [checked]="selectedDuplicates().has(file.documentId)" (change)="toggleDuplicateSelection(file.documentId)" class="mt-1 w-4 h-4 rounded text-red-600 focus:ring-red-500 border-[var(--border-color)] bg-[var(--bg-card)]">
                 <div class="flex-1 min-w-0">
                   <div class="text-[13px] font-medium text-[var(--text-secondary)] truncate" [title]="file.fileName">{{ file.fileName }}</div>
                   <div class="flex items-center gap-2 mt-0.5">
                     <span class="bg-[var(--bg-elevated)] text-[var(--text-muted)] text-[11px] font-medium px-1.5 py-0.5 rounded">{{ formatFileSize(file.fileSize || 0) }}</span>
                     <span class="text-[11px] text-[var(--text-muted)]" *ngIf="file.uploadedAt">Uploaded: {{ file.uploadedAt | date:'shortDate' }}</span>
                   </div>
                 </div>
               </label>
             </div>
