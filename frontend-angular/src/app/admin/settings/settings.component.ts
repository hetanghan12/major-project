      ...
      <div class="flex flex-1 min-h-0 gap-8">
        ...
        <div class="w-64 flex-shrink-0 flex flex-col gap-2">
          ...
          <button 
            ...
            class="w-full flex items-center gap-3 px-4 py-3 text-sm rounded-xl transition-colors"
          ...
          <button class="w-full flex items-center gap-3 px-4 py-3 text-sm rounded-xl font-medium text-gray-400 cursor-not-allowed transition-colors">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 flex flex-col min-h-0" *ngIf="!loading; else spinner">
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 flex-1 overflow-y-auto mb-6">
            <div class="p-8">
              ...
              <div *ngIf="activeTab === 'security'">
                <h2 class="text-base font-bold text-gray-900 mb-8 pb-4 border-b border-gray-100">Security & Authentication</h2>
                
                <div class="max-w-3xl space-y-10" *ngIf="settings.securitySettings">
                  
                  <!-- Section 1: Login Attempt Protection -->
                  <div class="space-y-6">
                    <h3 class="text-sm font-bold text-indigo-600 uppercase tracking-widest flex items-center gap-2">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                      1. Login Attempt Protection
                    </h3>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Max Login Attempts</label>
                        <input type="number" [(ngModel)]="settings.securitySettings.maxLoginAttempts" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors shadow-sm" placeholder="5">
                        <p class="text-[10px] text-gray-400 mt-2 font-medium">Recommended: 3-10 attempts</p>
                      </div>

                      <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Lock Duration (Minutes)</label>
                        <input type="number" [(ngModel)]="settings.securitySettings.lockDurationMinutes" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors shadow-sm" placeholder="15">
                        <p class="text-[10px] text-gray-400 mt-2 font-medium">Recommended: 15-30 minutes</p>
                      </div>
                    </div>
                  </div>

                  <!-- Section 2: File Upload Restrictions -->
                  <div class="space-y-6 pt-6 border-t border-gray-50">
                    ...
                    <div>
                      <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Allowed File Types</label>
                      <input type="text" [ngModel]="fileTypesString" (ngModelChange)="onFileTypesChange($event)" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors shadow-sm" placeholder="pdf, docx, txt, png, jpg">
                      <p class="text-[10px] text-gray-400 mt-2 font-medium">Separate extensions with commas (e.g. pdf, docx, png)</p>
                    </div>
                  </div>

                  <!-- Section 3: Session Security -->
                  <div class="space-y-6 pt-6 border-t border-gray-50 pb-10">
                    <h3 class="text-sm font-bold text-indigo-600 uppercase tracking-widest flex items-center gap-2">
                    ...
                    <div>
                      <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Session Timeout</label>
                      <select [(ngModel)]="settings.securitySettings.sessionTimeoutMinutes" class="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors shadow-sm bg-white">
                        <option [value]="30">30 minutes (High Security)</option>
                        <option [value]="60">60 minutes (Standard)</option>
                        <option [value]="240">240 minutes (Relaxed)</option>
                      </select>
                      <p class="text-[10px] text-gray-400 mt-2 font-medium">Users will be automatically logged out after this period of inactivity.</p>
                    </div>
                  </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex justify-end items-center gap-4 flex-shrink-0">
            <button class="px-6 py-2.5 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors" [disabled]="saving" (click)="loadSettings()">
              Reset
            </button>
            <button (click)="saveSettings()" [disabled]="saving" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-colors shadow-sm disabled:opacity-70 flex items-center">
              ...
              {{ saving ? 'Saving...' : (activeTab === 'security' ? 'Save Security Settings' : 'Save Changes') }}
            </button>
          </div>
        </div>
...
export class AdminSettingsComponent implements OnInit {
  ...
  // Computed helper for file types string representation
  get fileTypesString(): string {
    return this.settings?.securitySettings?.allowedFileTypes?.join(', ') || '';
  }

  // Helper to handle string back to array conversion
  onFileTypesChange(value: string) {
    if (this.settings.securitySettings) {
      this.settings.securitySettings.allowedFileTypes = value
        .split(',')
  ...
  async loadSettings() {
    try {
      ...
      // Ensure nested securitySettings object exists to avoid template errors
      if (!this.settings.securitySettings) {
